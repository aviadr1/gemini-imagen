#!/usr/bin/env python3
"""
Uninstaller for gemini-imagen standalone installations.

This script removes gemini-imagen that was installed via the standalone installer.
...
def get_install_receipt_path() -> Path:
    ...
    if os_name == "windows":
        config_dir = (
            Path(os.environ.get("LOCALAPPDATA", Path.home() / "AppData" / "Local")) / "imagen"
        )
    else:
        # Unix (Linux/macOS)
...
def clean_path_unix(wrapper_dir: Path) -> bool:
    """Remove gemini-imagen from PATH in Unix shell profiles."""
    home = Path.home()
    profiles = [
        home / ".bashrc",
        home / ".bash_profile",
    ...
    comment_pattern = re.compile(r"# Added by gemini-imagen installer\n")
...
def clean_path_windows(wrapper_dir: Path) -> bool:
    """Remove gemini-imagen from PATH in Windows registry."""
    try:
        import winreg
...
def main() -> int:
    ...
    print(f"\n{BOLD}gemini-imagen Uninstaller{RESET}\n")
    ...
    if receipt is None:
        ...
        print("If you installed via pip, run: pip uninstall gemini-imagen")
        print()
    ...
    if receipt:
        ...
        if os_name == "windows":
            ...
            venv_path = local_appdata / "gemini-imagen"
            wrapper_path = local_appdata / "Programs" / "imagen"
        else:
            data_home = Path(os.environ.get("XDG_DATA_HOME", Path.home() / ".local" / "share"))
            venv_path = data_home / "gemini-imagen"
    ...
    print(
        f"  Wrapper script:      {wrapper_path / 'imagen' if os_name != 'windows' else wrapper_path / 'imagen.bat'}"
    )
    ...
    wrapper_file = wrapper_path / "imagen.bat" if os_name == "windows" else wrapper_path / "imagen"
